...
module Either =
    ...
    let bimap (f: 'a -> 'a0) (g: 'b -> 'b0) (e: Either<'a, 'b>) =
        match e with
        | Left a -> Left(f a)
        | Right b -> Right(g b)
    ...
    let bind (f: 'b -> Either<'a, 'a0>) (e: Either<'a, 'b>) : Either<'a, 'a0> =
        match e with
        | Left a -> Left a
        | Right b -> f b

    let right (e: Either<'a, 'b>) =
        match e with
        | Left _ -> None
        | Right b -> Some b

    let left (e: Either<'a, 'b>) =
        match e with
        | Left a -> Some a
        | Right _ -> None

    let fold (l: 'a -> 'a0) (r: 'b -> 'a0) (e: Either<'a, 'b>) =
        match e with
        | Left a -> l a
        | Right b -> r b